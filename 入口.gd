...
func _更新拖拽落点标识(落点信息: Dictionary):
	...
	if 是否子级落点:
		...
		_拖拽占位块.position = Vector2(2.0, 行区域.position.y + 行区域.size.y)
		_拖拽占位块.size = Vector2(max(树状菜单.size.x - 4.0, 4.0), 占位高度)
		_拖拽占位块.visible = true
	...
	if 落点段 <= 0:
		...
		_拖拽占位块.position = Vector2(2.0, 行区域.position.y)
		_拖拽占位块.size = Vector2(max(树状菜单.size.x - 4.0, 4.0), 占位高度)
		_拖拽占位块.visible = true
	else:
		_拖拽下侧横线.position = Vector2(横线左侧, 行区域.position.y + 行区域.size.y - 横线厚度)
...
func _统计拖拽可视行数(拖拽数据) -> int:
	if !(拖拽数据 is Array) or 拖拽数据.is_empty():
		return 1
	var 根项列表: Array = []
